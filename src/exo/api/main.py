...
# Bare-process patterns in runner_stderr / system logs: no loguru prefix,
# emitted directly by the OS/signal handler. Treat as synthetic ERROR.
_BARE_SEGFAULT_PATTERNS = re.compile(
    r"(?:Segmentation fault|Killed(?: -)?\s*process|OOM\s*killed|"
    r"out of memory|SIGSEGV|SIGABRT|SIGKILL|ABRT)",
    re.IGNORECASE,
)
...
def _parse_log_errors(content: str, source_log: str, context_lines: int = 5) -> list[LogErrorEntry]:
    ...
    Bare segfault/OOM killer lines (no loguru prefix) are surfaced as
    synthetic ERROR entries with ``source="(signal)"``.
    """
    ...
    i = 0
    while i < len(lines):
        # Bare segfault/OOM lines never match _LOG_LINE_RE — surface them anyway
        if _LOG_LINE_RE.match(lines[i]) is None and _BARE_SEGFAULT_PATTERNS.search(lines[i]):
            bad = lines[i].strip()
            ctx_before = [lines[k] for k in range(max(0, i - context_lines), i)]
            ctx_after = [lines[k] for k in range(i + 1, min(len(lines), i + 1 + context_lines))]
        ...
        while j < len(lines) and _LOG_LINE_RE.match(lines[j]) is None:
            # Stop collapsing if the continuation is itself a bare segfault/OOM
            # line — it becomes its own entry on the next outer iteration.
            if _BARE_SEGFAULT_PATTERNS.search(lines[j]):
                break
            extra = lines[j].strip()
...
class API:
    ...
    def _setup_routes(self) -> None:
        ...
        # Local {name}/raw must register BEFORE the proxy {node_id}/{name}
        # routes so Starlette's trie matches /v1/logs/main/raw → get_log_raw
        # instead of treating "main" as a node_id.
        self.app.get("/v1/logs/{name}/raw")(self.get_log_raw)
        self.app.get("/v1/logs/{node_id}/{name}")(self.get_node_log_tail)
        self.app.get("/v1/logs/{node_id}/{name}/raw")(self.get_node_log_raw)
    ...
    async def promote_master(self, node_id: NodeId) -> PromoteMasterResponse:
        ...
        if self.state.instances:
            raise ApiError(
                status_code=409,
                detail=(
                    "Cannot promote master while instances are running "
                    "-- every node's worker restarts when master changes. "
                    "Eject running models first."
                ),
                error_code="INSTANCES_RUNNING",
            )
    ...
    async def get_feature_flags(self) -> dict[str, bool]:
        return {
            ...
            "server_side_tools": tools_enabled(),
        }
    ...
    # ── Per-node log proxy ───────────────────────────────────────────────
    # ``_node_api_base`` looks up the advertised API host:port for a node
    # id from ``State.node_identities`` (populated from NodeIdentity at
    # discovery-time).  The proxy level avoids baking httpx into every
    # simple log-tail case while letting the dashboard show any node's
    # logs through the master without CORS or node-endpoint discovery.
    def _node_api_base(self, node_id: str) -> str | None:
        infos = getattr(self.state, "node_identities", {}) or {}
        ident = infos.get(node_id)  # pyright: ignore[reportAny]
        if ident is None:
            return None
        host = str(getattr(ident, "api_host", "") or "")
        port = int(getattr(ident, "api_port", 0) or 0)
        if not host or not port or host == "0.0.0.0":
            return None
        return f"http://{host}:{port}"

    async def get_node_log_tail(
        self,
        node_id: str,
        name: str,
        lines: int = 1000,
    ) -> LogTailResponse:
        """Proxy ``GET /v1/logs/{node_id}/{name}`` to the target node's API.

        Routes file whitelist, auth, and pagination checks are identical to
        :meth:`get_log_tail`; only the I/O is remote.  The master proxies
        so the dashboard (which talks only to the master) can show any
        node's logs without CORS or endpoint discovery on the client.
        """
        if _LOG_FILES.get(name) is None:
            raise HTTPException(status_code=404, detail=f"Unknown log: {name}")
        base = self._node_api_base(node_id)
        if base is None:
            raise HTTPException(
                status_code=404,
                detail=f"Unknown or unreachable node_id: {node_id}",
            )
        import httpx  # lazy — avoids importing until first proxy call

        url = f"{base}/v1/logs/{quote(name, safe='')}?lines={int(lines)}"
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            detail = "Remote log error"
            try:
                detail = str(exc.response.json().get("detail", detail))  # pyright: ignore[reportAny]
            except Exception:
                pass
            raise HTTPException(
            ...
            raise HTTPException(
                status_code=502,
                detail=f"Could not reach node {node_id} ({base}): {exc}",
            )
    ...
    async def get_node_log_raw(
        self,
        node_id: str,
        name: str,
    ) -> Response:
        """Proxy ``GET /v1/logs/{node_id}/{name}/raw`` to the target node."""
        if _LOG_FILES.get(name) is None:
            raise HTTPException(status_code=404, detail=f"Unknown log: {name}")
        base = self._node_api_base(node_id)
        if base is None:
        ...
        url = f"{base}/v1/logs/{quote(name, safe='')}/raw"
        try:
            async with httpx.AsyncClient(timeout=5.0, follow_redirects=True) as client:
                ...
                if resp.status_code == 404:
                    detail = "Remote log error"
                    try:
                        detail = str(resp.json().get("detail", detail))  # pyright: ignore[reportAny]
                    except Exception:
                        detail = resp.text  # pyright: ignore[reportAny]
