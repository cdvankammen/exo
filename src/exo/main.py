...
class Node:
    ...
    async def create(cls, args: "Args") -> Self:
        ...
        election = Election(
            ...
            is_candidate=not args.no_master,
    ...
    async def run(self):
        async with self._tg as tg:
            ...
            def _request_shutdown(*_: object) -> None:
                ...
                try:
                    # anyio TaskGroup has no cancel_tasks(); the cancel scope
                    # is the correct graceful-cancellation API.
                    anyio.from_thread.run(tg.cancel_scope.cancel)
                except Exception:
                    logger.opt(exception=True).warning("Failed to schedule task cancellation")
...
def main_inner(args: "Args"):
    ...
    if args.metrics_port:
        os.environ["EXO_METRICS_PORT"] = str(args.metrics_port)
        from exo.shared.metrics import init_metrics
        init_metrics()
...
class Args(FrozenModel):
    ...
    def parse(cls) -> Self:
        ...
        parser.add_argument(
            ...
            dest="metrics_port",
            default=None,
            help="Port for Prometheus /metrics endpoint (or set EXO_METRICS_PORT)",
        )
        parser.add_argument(
            "--namespace",
            type=str,
