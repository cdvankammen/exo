...
async def _get_thunderbolt_devices() -> set[str] | None:
    ...
    try:
        ...
        logger.warning("networksetup -listallhardwareports timed out after 5s")
        return None
...
async def _get_bridge_services() -> dict[str, str] | None:
    ...
    try:
        ...
        logger.warning("networksetup -listnetworkserviceorder timed out after 5s")
        return None
    if result.returncode != 0:
        logger.warning(
...
async def _get_bridge_members(bridge_device: str) -> set[str]:
    ...
    try:
        ...
        logger.warning(f"ifconfig {bridge_device} timed out after 5s")
        return set()
    if result.returncode != 0:
...
async def _is_service_enabled(service_name: str) -> bool | None:
    ...
    Returns True if enabled, False if disabled, None on error or timeout.
    """
    try:
        with fail_after(5):
            result = await anyio.run_process(
        ...
        logger.warning(
            f"networksetup -getnetworkserviceenabled '{service_name}' timed out after 5s"
        )
        return None
    if result.returncode != 0:
        logger.warning(
...
class NodeDiskUsage(TaggedModel):
    ...
    async def gather(cls) -> Self:
        try:
            ...
            logger.warning(
                f"Disk usage query for {EXO_DEFAULT_MODELS_DIR} timed out after 10s"
            )
            # Return a zero-usage placeholder so the node still reports
            from exo.shared.types.memory import Memory

            return cls(
                disk_usage=DiskUsage(total=Memory.from_mb(0), available=Memory.from_mb(0))
            )
...
async def _gather_iface_map() -> dict[str, str] | None:
    try:
        ...
        logger.warning("networksetup -listallhardwareports timed out after 5s (iface_map)")
        return None
    if proc.returncode != 0:
        return None
...
class InfoGatherer:
    ...
    async def run(self):
        async with self._tg as tg:
            ...
            # Send identity-building events at startup so the node appears
            # in nodeIdentities immediately rather than waiting up to 60s for
            # the first periodic _monitor_* loop iteration.  Without this,
            # the node shows up in topology (from NodeConfig) but with an
            # empty nodeIdentities entry — the "intermittent identity"
            # bug seen on mini2 (t_932ecdce, same class as Linux 9519a896).
            try:
                await self.info_sender.send(await MiscData.gather())
            except Exception as e:
                logger.opt(exception=e).warning(
                    "Error gathering misc data at startup"
                )

            try:
                static_info = await StaticNodeInformation.gather()
                if static_info is not None:
                    await self.info_sender.send(static_info)
            except Exception as e:
                logger.opt(exception=e).warning(
                    "Error gathering static node info at startup"
                )
