...
def instance_to_reset_backoff(
    ...
    if not isinstance(event, RunnerStatusUpdated):
        return None
    if not isinstance(event.runner_status, (RunnerReady, RunnerRunning, RunnerDegraded)):
        return None
...
def _create_runner(
    ...
    for instance in instances.values():
        ...
        # Guard against re-spawning a runner while the system is still under
        # severe memory pressure.  A runner that OOM-leaked its memory and died
        # will be recreated on the next plan() tick; if the leak hasn't been
        # reclaimed yet (< ~3 min), the replacement will immediately OOM too,
        # burning through the instance retry budget for no benefit.  Skip the
        # create until memory drops below 90 % so the kernel has time to
        # reclaim pages / the user has time to intervene.
        try:
            mem = virtual_memory_statistics()
            if mem.used_fraction > 0.90:
                logger.warning(
                    f"Skipping runner creation for instance {instance.instance_id}: "
                    f"memory pressure too high ({mem.used_fraction:.1%} used, "
                    f"{mem.available_bytes // (1024*1024)} MiB available). "
                    f"Waiting for pressure to drop below 90%."
                )
                continue
        except Exception:
            # If memory stats fail (e.g. psutil glitch on a new OS), don't
            # let the guard itself block runner creation — fail open.
            pass
