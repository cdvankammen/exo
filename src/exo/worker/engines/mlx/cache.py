...
# Memory thresholds routed through SettingsManager:
# persisted UI override > env > in-code default.
_settings = get_settings_manager()

_memory_threshold_raw = _settings.get_value("EXO_MEMORY_THRESHOLD")
_MEMORY_THRESHOLD = (
    float(_memory_threshold_raw)
    if _memory_threshold_raw is not None
    else _default_memory_threshold()
)
...
_prefill_threshold_raw = _settings.get_value("EXO_PREFILL_MEMORY_THRESHOLD")
_PREFILL_MEMORY_THRESHOLD = (
    float(_prefill_threshold_raw)
    if _prefill_threshold_raw is not None
    else max(0.0, _MEMORY_THRESHOLD - 0.10)
)
...
# in-place grows actually use (they always extend from the tip). (Port of PR
# #2137.)
...
class KVPrefixCache:
    def __init__(self, group: mx.distributed.Group | None, model_id: str = ""):
        ...
        self._disk_enabled = (
            get_settings_manager().get_value("EXO_KV_DISK_PERSISTENCE", "0") == "1"
        )
        self._disk_dir = (
