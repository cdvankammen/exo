...
def shard_and_load(
    ...
    match shard_metadata:
        case TensorShardMetadata():
            if _EXPERT_PARALLEL_ENABLED:
                logger.info(f"loading model from {model_path} with EXPERT parallelism")
                model = yield from expert_auto_parallel(model, group)
            else:
