...
def _value_has_markup(value: object) -> bool:
    if isinstance(value, dict):
        items = cast("dict[object, object]", value)
        return any(
            (isinstance(k, str) and _contains_markup(k)) or _value_has_markup(v)
            for k, v in items.items()
        )
    if isinstance(value, list):
        items = cast("list[object]", value)
        return any(_value_has_markup(item) for item in items)
    if isinstance(value, str):
        return _contains_markup(value)
    return False


def _has_residual_markup(parsed_call: object) -> bool:
    if not isinstance(parsed_call, dict):
        return False
    call = cast("dict[str, object]", parsed_call)
    name = call.get("name")
    if isinstance(name, str) and _contains_markup(name):
        return True
    return _value_has_markup(call.get("arguments"))
