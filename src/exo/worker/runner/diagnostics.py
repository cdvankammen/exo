...
# MLX / Metal allocator failures surface as C++ exceptions on stderr:
#   libc++abi: terminating with uncaught exception of type
#     std::runtime_error: [METAL] Failed to allocate ... bytes
#     std::runtime_error: [METAL] Insufficient memory to allocate ...
#   std::bad_alloc
_OOM_LINE_RE = re.compile(
    r"(?:Failed to allocate|Insufficient memory|bad_alloc|out of memory)",
    re.IGNORECASE,
)
...
class RunnerOutOfMemory(BaseRunnerDiagnostic):
...
class RunnerDiagnosticCollector:
    ...
    def _classify_line(
        ...
        if oom_error := _parse_oom_error(line, evidence):
            return oom_error
...
def _parse_oom_error(
    line: str, evidence: tuple[str, ...]
) -> RunnerOutOfMemory | None:
    """Detect MLX/Metal out-of-memory errors from stderr.
    
    MLX raises C++ exceptions that surface on stderr as:
      - [METAL] Failed to allocate N bytes
      - [METAL] Insufficient memory to allocate ...
      - std::bad_alloc
      - out of memory (various capitalizations)
    """
    if not _OOM_LINE_RE.search(line):
        return None

    # Try to extract peak memory from the line if present
    bytes_match = re.search(r"(\d+)\s*bytes?", line, re.IGNORECASE)
    peak_bytes = int(bytes_match.group(1)) if bytes_match else 0

    return RunnerOutOfMemory(
        message=f"GPU out of memory: {line.strip()}",
        evidence=evidence,
